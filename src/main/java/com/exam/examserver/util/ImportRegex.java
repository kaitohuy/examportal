...
public final class ImportRegex {
        ...
        // ↑ phần sau dấu chấm cho 1–2 chữ số; bạn có thể siết thành [1-9]\d để tránh
        // "1.0"

        // 3) Gộp lại: "1." HOẶC "1.1 / 1.1.1"
        private static final String NUM_ANY = "(?:" + NUM_SIMPLE_DOT + "|" + NUM_HIER + ")";

        // 3b) Prefix chữ cho câu hỏi: OT, NH, TC..., có thể có/không dấu chấm, có/không
        // space
        // Ví dụ match: "O.1.2", "OT.1.2", "NH1.1", "TC 2.3"
        private static final String Q_PREFIX = "(?:[A-Za-z]{1,3}\\.?\\s*)?";

        private static final String Q_HEADER_NUMERIC_CORE = Q_PREFIX + NUM_ANY;
        ...
        private static final String HEADER_BREAK_CORE = "(?:"
                        + HL + "(?:question|câu\\s?hỏi|câu|bài)" + HL + "\\s*(?:số)?" + HL + "\\s*\\d+(?:\\.\\d+)*" + HL
                        + "\\s*[:.)-]?"
                        + "|" + HL + "q" + HL + "[.:\\-]?" + HL + "\\s*\\d+" + HL + "\\s*[:.)-]?"
                        + ")";

        private static final String ANSWER_CORE = HL + "(?:đáp\\s*án|answer|giải\\s*thích|gợi\\s*ý)";

        // CHỈ option mới yêu cầu A–D HOA; có thể có {hl} ngay trước chữ cái
        private static final String OPT_HEAD_CORE = "(?:\\{hl\\}\\s*)?[A-D]\\s*[.\\)]\\s+\\S";
        ...
        /* ===== Public patterns ===== */
        public static final Pattern P_SPLIT_BY_HEADER = Pattern.compile("(?=^\\s*" + HEADER_CORE + ")", RXFM);

        public static final Pattern P_HEADER_ANCHORED = Pattern.compile("^\\s*" + HEADER_CORE, RXFM);
        ...
        public static final Pattern P_ANSWER_LABEL = Pattern.compile("^\\s*(" + ANSWER_CORE + ")\\s*[:\\-]?\\s*(.+)$",
                        RXFM);

        public static final Pattern P_IMAGE_PLACEHOLDER = Pattern.compile("\\{\\{image(\\d+)\\}\\}");

        // dùng khi ĐÃ biết là MC
        private static final Pattern BR_OPT = Pattern.compile("(?<![\\r\\n])\\s+(?=" + OPT_HEAD_CORE + ")");

        /* ===== Inline breakers ===== */
        private static final Pattern BR_HEADER = Pattern.compile("(?<![\\r\\n])\\s+(?=" + HEADER_BREAK_CORE + ")", RXF);
        private static final Pattern BR_ANSWER = Pattern.compile("(?<![\\r\\n])\\s+(?=" + ANSWER_CORE + "\\s*[:\\-])",
                        RXF);

        /* ===== Chapter ===== */
        private static final String CHAPTER_CORE = HL + "(?:chương|chuong|chapter)" + HL + "\\s*(?:số)?" + HL
                        + "\\s*(\\d+)" + HL + "\\s*[:.)-]?";

        public static final Pattern P_SPLIT_BY_CHAPTER = Pattern.compile("(?=^\\s*" + CHAPTER_CORE + ")", RXFM);

        public static final Pattern P_CHAPTER_ANCHORED = Pattern.compile("^\\s*" + CHAPTER_CORE + ".*$", RXFM);

        private static final String CHAPTER_BREAK_CORE = HL + "(?:chương|chuong|chapter)" + HL + "\\s*(?:số)?" + HL
                        + "\\s*\\d+(?:\\.\\d+)*" + HL + "\\s*[:.)-]?";

        private static final Pattern BR_CHAPTER = Pattern.compile("(?<![\\r\\n])\\s+(?=" + CHAPTER_BREAK_CORE + ")",
                        RXF);
        ...
        public static final Pattern P_BULLET_LINE = Pattern
                        .compile("^\\s*(?:[-•]\\s+|[a-dA-D][\\)\\.]\\s+|\\([a-dA-DA-D]\\)\\s+).*");
        ...
        public static String extractNumericTypeCode(String block) {
                ...
                if (m.find()) {
                        ...
                        // Chuẩn hoá: luôn thêm dấu chấm vào cuối nếu chưa có
        ...
        public static final Pattern P_SUBITEM_HEADER = Pattern
                        .compile("(?mi)^\\s*(?:\\(\\s*([a-dA-D])\\s*\\)|([a-dA-D])\\s*[\\)\\.:])\\s*");
        ...
        public static final Pattern P_BARE_LTX_TOKEN = Pattern
                        .compile("([\\p{L}\\p{N}\\)\\]\\}])(?:_(\\{[^}]+\\})|\\^(\\{[^}]+\\}))+");
        ...
        public static String normalizeStem(String s) {
                ...
                // 1) Nếu CHỈ là mã số (ví dụ "2.1.1." hoặc "2.1.1" hoặc "{hl}2.1{hl}"), coi như
                // KHÔNG có đề chung
                // (case này trước khi strip để bắt cả chuỗi "trần")
                String onlyNumRegex = "^\\s*(?:\\{/?hl\\}\\s*)?" + Q_PREFIX + NUM_ANY +
                                "(?:\\s*(?:\\{/?hl\\}\\s*)*[\\.)\\-:]?)?\\s*$";
                if (t.matches(onlyNumRegex))
                        return null;
        ...
        private static final Pattern P_CLONE_PREFIX = Pattern.compile("(?i)^C\\s*(\\d+)\\.(.+)$");

        /** Trích mã từ block; trả về chuỗi đã trim (giữ nguyên dấu ')'). */
        /**
         * Trích mã từ block; trả về chuỗi đã chuẩn hoá (giữ hoặc bổ sung ')' nếu
         * thiếu).
         */
        ...
        public static AbstractMap.SimpleEntry<Integer, String> parseCloneCode(String code) {
                ...
                if (!m.find())
                        return new AbstractMap.SimpleEntry<>(null, code.trim());
                Integer idx = null;
                try {
